...
import { extract } from "jsr:@std/archive";
import { readAll } from "https://deno.land/std@0.224.0/io/read_all.ts";
import { parsePPTX, validateMethod } from "./pptx_parser.tsx";
const app = new Hono();
...
app.post('/make-server-aac39e77/methods', async (c) => {
  try {
    ...
    const newMethod = {
      ...
      imageUrl: method.imageUrl || '/grafik-1.png', // Default image
...
// ===== UPLOAD PPTX =====
app.post('/make-server-aac39e77/upload-pptx', async (c) => {
  try {
    console.log('📤 PPTX Upload started');
    ...
    if (!file.name.endsWith('.pptx')) {
      return c.json({ error: 'Nur .pptx Dateien werden unterstützt' }, 400);
    }
    ...
    // Parse PPTX and extract method
    console.log('🔍 Parsing PPTX...');
    const extractedMethods = await parsePPTX(fileBuffer);

    if (!extractedMethods || extractedMethods.length === 0) {
      return c.json({ error: 'Keine Methode in der PowerPoint gefunden' }, 400);
    }

    const method = extractedMethods[0];

    if (!validateMethod(method)) {
      return c.json({ error: 'Extrahierte Methode ist unvollständig' }, 400);
    }
    ...
    const newMethod = {
      ...
      id: `method-${Date.now()}`,
      createdAt: new Date().toISOString(),
      imageUrl: '/grafik-1.png', // Default image
      contactPerson: {
        name: 'Ansprechpartner',
        role: 'Methodenexperte',
        email: 'kontakt@beispiel.de'
      },
      examples: method.tips?.slice(0, 2) || []
    };

    methods.push(newMethod);
    await kv.set('methods:all', methods);

    console.log('✅ Method saved:', newMethod.title);

    return c.json({
      success: true,
      method: newMethod,
      message: 'PowerPoint erfolgreich analysiert und Methode erstellt!'
    });

  } catch (error) {
    console.error('❌ Error uploading PPTX:', error);
    return c.json({
      error: 'PowerPoint-Analyse fehlgeschlagen',
